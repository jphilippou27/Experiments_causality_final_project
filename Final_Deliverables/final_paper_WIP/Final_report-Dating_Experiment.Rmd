...
```{r}
model_base = lm(matches ~ md + bs + phd  + female, data = df)
summary(model_base)
...
```{r}
model_gender = lm(matches ~ md + bs + phd + noedu + female + female*matches, data = df)
...
library(stargazer)
model_MD_base = lm(matches ~ md, data = df)
model_BS_base = lm(matches ~ bs, data = df)
model_noedu_base = lm(matches ~ noedu, data = df)
model_PhD_base = lm(matches ~ phd, data = df)

#Change type to "latex" for knitting to pdf
stargazer(model_MD_base, model_BS_base, model_noedu_base, model_PhD_base,type = "text", report = "vcs*", single.row = F, column.labels = c("MD","BS", "No Edu", "PhD"), title ="Comparison of treatments")
#cat("\n\n\\pagebreak\n")
```
## Age impact from Kavlin

```{r}
noedu.model <- lm(age ~ noedu, data=df_female)
robustSEs(noedu.model)
bs.model <- lm(age ~ bs, data=df_female)
robustSEs(bs.model)
md.model <- lm(age ~ md, data=df_female)
robustSEs(md.model)
phd.model <- lm(age ~ phd, data=df_female)
robustSEs(phd.model)
```
