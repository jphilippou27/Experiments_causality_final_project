...
```{r}
sex.model <- lm(age ~ female, data=df)
robustSEs(sex.model)
education_level.model <- lm(age ~ bs + md + phd, data=df)
robustSEs(education_level.model)
city.model <- lm(age ~ houston + losangeles + newyork + philadelphia + phoenix + sanantonio + sandiego, data=df)
...
WE CAN REMOVE THE VISUAL ON THE NEXT FEW REGRESSIONS I LEFT THEM IN CASE WE WANT TO TWEAK ANYTHING
Location impact: (chicago is the base)
```{r}
...
```{r}
df_female = subset(df, df$female == 1)
model_location2 = lm(matches ~ md + bs + phd  + losangeles + houston + newyork + phoenix + sandiego +sanantonio +philadelphia +losangeles*md +losangeles*bs+losangeles*phd , data = df_female)
summary(model_location2)
```
...
```{r}
model_details = lm(matches ~ md + bs + phd  +  num_ig + school + job, data = df_female)
summary(model_details)
```

```{r}
df_female = subset(df, df$female == 1)
model_all = lm(matches ~ md + bs + phd  + female + age + losangeles + houston + newyork + phoenix + sandiego +sanantonio +philadelphia + num_ig + school + job  , data = df_female)
summary(model_all)
```

Added just in case we want to run the treatments separately:
